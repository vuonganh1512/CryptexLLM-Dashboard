...
def simple_backtest(df: pd.DataFrame, fee_bps: float = 0.0, threshold: float = 0.0) -> pd.DataFrame:
    """
    df needs columns: Date, actual_return, pred_return
    Rule: long if pred_return > threshold else flat
    """
    x = df.copy().sort_values("Date").reset_index(drop=True)

    x["position"] = (x["pred_return"] > threshold).astype(int)
